...
# KBL 전반전 점수 통계
def kbl_FirstHalf_Data(homeName, awayName):
    ...
    for year in yearList:
        ...
        url = f'https://api.kbl.or.kr/leagues/S{str(year)}G01/vs/team-stats?teamA={TEAM_NAME[homeName]}&teamB={TEAM_NAME[awayName]}'
    ...
    scoreList = []
    scoreList.append(int(homeScore/4))
    scoreList.append(int(awayScore/4))
...
# 배트맨 NBA 2019~2023 상대 점수차 (홈, 원정 동일 위치)
...
# 배트맨 KBL 데이터 크롤링
def betman_KBL_DataLoad(year):

    fileName = kbl_mkdir_path + '배트맨_KBL_' + str(year) + '년도_데이터.xlsx'
    ...
    for i in range(200):
        ...
        with HTMLSession() as s:
            ...
            for li in output:
                ...
                if _type != 'BK' or _matchResult == None or _homeHandy != 0.0 or _leagueName != 'KBL': 
                    continue
...
def betman_DailyLineup_Analysis(homeName, awayName):
    ...
    elif awayName == '울산모비': awayName = '울산현대모비스'
    
    if homeName == '한국가스': homeName = '대구한국가스공사'
    elif awayName == '한국가스': awayName = '대구한국가스공사'
    ...
    for row in team_ws.rows:
        if row[1].value == homeName and row[5].value != None:
            homeTeamScore = int(row[6].value)/4

        if row[1].value == awayName and row[5].value != None:
            awayTeamScore = int(row[6].value)/4
    ...
    gapScore = int(betman_Difference_Score(homeName,awayName))

    halfList = kbl_FirstHalf_Data(homeName, awayName)
...
if __name__ == '__main__':
    ...
    # 배트맨 라인업
    betman_DailyLineup_Analysis('창원LG','서울SK')
